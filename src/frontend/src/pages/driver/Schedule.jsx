  ...
  useEffect(() => {
    const fetchWeek = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/driverschedule/driver/${driverId}?from=${from}&to=${to}`);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-6 pb-6">
        ...
        {loading ? (
          <div className="bg-white p-6 rounded-lg shadow text-center">Đang tải...</div>
        ) : (
          <>
            {weekSchedules[selectedDay] && weekSchedules[selectedDay].length === 0 ? (
              <div className="bg-white p-6 rounded-lg shadow text-center text-gray-500">
                Không có lịch làm việc trong ngày này.
              </div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                {weekSchedules[selectedDay].map((item) => (
                  <div key={item.schedule_id} className="bg-white rounded-lg shadow p-4 flex flex-col justify-between hover:shadow-lg transition">
                    ...
                    <div className="mb-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className={`text-xs px-2 py-1 rounded ${statusColor(item.status)}`}>
                          {item.status}
                        </div>
                      </div>
                      <div className="text-gray-600 text-sm mt-1">{item.route_name}</div>
                    </div>
                    ...
                    <div className="text-right">
                      <button
                        ...
                        onClick={() => window.location.href = `/driver/schedule/${item.schedule_id}`}
                      >
                        Xem chi tiết
                      </button>
                    </div>
