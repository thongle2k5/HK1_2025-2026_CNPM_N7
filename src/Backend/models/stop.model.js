...
export const StopModel ={
    ...
    getStopsByRouteId: async (routeId) =>{
        const [rows] = await db.query('select stop.*,stop_route.order,stop_route.expected_arrive_time from stop_route left join stop on stop_route.stop_id = stop.stop_id where stop_route.route_id = ?',[routeId]);
        return rows;
    }
